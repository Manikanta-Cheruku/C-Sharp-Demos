...
/** A SortedList in C# is a collection of key-value pairs that are sorted by the keys. It combines the features of a Hashtable and a SortedDictionary, 
where the elements are accessible by both key and index and are automatically sorted by the key. SortedList is part of the System.Collections namespace.

Let's create a simple example to demonstrate how to use a SortedList in C#. This example will cover adding elements, accessing elements, modifying elements,

Scenario: Managing a Sorted List of Students
In this example, we will create a SortedList to store students' information where the key is the student ID (integer) and the value is the student name (string).
The list will automatically sort students by their IDs.**/
namespace SortedArrayListDemo
{
